...
MicroPython driver for the Adafruit HDC302x Precision Temperature/Humidity breakout

* Author(s): Liz Clark (Adapted for MicroPython)
...
class HDC302x:
    ...
    @auto_mode.setter
    def auto_mode(self, mode: Union[str, int]) -> None:
        """
    ...
    def _write_command(self, command: int) -> None:
        self.i2c.writeto(self.address, bytes([command >> 8, command & 0xFF]))

    def _write_command_data(self, command: int, data: int) -> None:
        crc = self._calculate_crc8(struct.pack(">H", data))
        self.i2c.writeto(self.address, bytes([command >> 8, command & 0xFF, data >> 8, data & 0xFF, crc]))

    def _read_command(self, command: int) -> int:
        # Send command with repeated start (False for stop)
        self.i2c.writeto(self.address, bytes([command >> 8, command & 0xFF]), False)
    ...
    def _send_command_read_trh(self, command: int) -> Tuple[float, float]:
        ...
        if self._calculate_crc8(result[:2]) != result[2] or self._calculate_crc8(result[3:5]) != result[5]:
            raise RuntimeError("CRC check failed")
    ...
    @staticmethod
    def _calculate_crc8(data: Union[bytes, bytearray]) -> int:
